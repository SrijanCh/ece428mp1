...
func (t *Beat_table) Reval_table(node_hash int, mem_table memtable.Memtable){
	...
	t.mu.Lock()
	for i := 0; i < 4; i++{
		if _, ok := t.table[neighbors[0]]; ok { //Node is in there
			newtable[neighbors[0]] = t.table[neighbors[0]]
		}else{
			newtable[neighbors[0]] = 0
		}
	}
	t.table = newtable
	t.mu.Unlock()
